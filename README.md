...
> A Node.js/Express backend monorepo with microservices for **Auth** (user authentication, address management, and token blacklisting), **Product** (product creation with image uploads), and **Cart** (user cart management).
...
|---------|-----------|--------------|----------------|
| Auth    | `Auth/`   | `3000`       | User registration, username/email login, logout blacklisting, profile, address management, and registration events |
...
- Token blacklisting on logout with Redis
- Protected `GET /me` endpoint to fetch the current user
- Address management: add, list, and remove user addresses
- Publishes a registration event to RabbitMQ for downstream services
...
| RabbitMQ    | ≥ 3.x    | required by the Auth service for registration events |
...
| `POST` | `/api/auth/login` | No | Login and receive a JWT cookie; identifier can be username or email |
